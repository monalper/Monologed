...
function MovieList() {
    ...
    useEffect(() => {
        const fetchPopularMovies = async () => {
            ...
            try {
                ...
                console.error("Popüler filmleri çekerken hata:", err);
                setError('Popüler filmler yüklenirken bir sorun oluştu.'); setMovies([]);
    ...
    return (
        <section className="relative group/section" aria-labelledby="popular-movies-heading">
            {/* Bölüm Başlığı */}
            <div className="flex items-center justify-between mb-4">
                {/* *** Başlık t() ile değiştirildi *** */}
                <h2 id="popular-movies-heading" className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-gray-100">
                   {t('homepage_popular_movies_title')}
                </h2>
            </div>
            ...
            {!loading && !error && movies.length > 0 && (
                 <div className="relative">
                    <div
                        ...
                        className="flex space-x-4 overflow-x-auto scroll-smooth pb-4 -mb-4 hide-scrollbar" // overflow-x-auto önemli
                    >
                        {movies.map(movie => (
                    ...
                    {/* Kaydırma Okları */}
                     <>
                         {canScrollLeft && (
                             <button
                                 onClick={() => scroll(-2)}
                                 className="absolute -left-4 top-1/2 -translate-y-1/2 z-20 p-2 bg-white dark:bg-gray-800/80 backdrop-blur-sm hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white rounded-full shadow-md transition-all duration-200 opacity-0 group-hover/section:opacity-100 focus:opacity-100"
                         ...
                         {canScrollRight && (
                             <button
                                 onClick={() => scroll(2)}
                                 className="absolute -right-4 top-1/2 -translate-y-1/2 z-20 p-2 bg-white dark:bg-gray-800/80 backdrop-blur-sm hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white rounded-full shadow-md transition-all duration-200 opacity-0 group-hover/section:opacity-100 focus:opacity-100"
             ...
             {!loading && !error && movies.length === 0 && (
                 <div className="h-56 flex items-center justify-center text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800/50 rounded-lg">
                     {t('error_no_popular_movies', 'Popüler film bulunamadı.')} {/* Çeviri eklendi */}
                 </div>
             )}
        </section>
    );
