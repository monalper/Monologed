...
function WatchlistPreviewPage() {
        ...
        const fetchWatchlist = async () => {
            ...
            try {
                ...
                if (error.response?.status === 404) {
                    setError(t('watchlist_preview_error_not_found'));
                } else {
                    setError(t('watchlist_preview_error_load'));
                }
                setWatchlistItems([]);
            } finally {
                setLoading(false);
    ...
    const handleAddToWatchlist = async (itemId, contentId, contentType) => {
        ...
        try {
            ...
            // Watchlist'e eklendiğinde kullanıcının watchlist'inden kaldır
            setWatchlistItems(prevItems => prevItems.filter(item => item.itemId !== itemId));
        } catch (error) {
            console.error("Watchlist'e ekleme hatası:", error);
        } finally {
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 border-b border-gray-200 dark:border-gray-700 pb-4">
                <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100">
                    {t('watchlist_preview_title', { username })}
                </h1>
                {!loading && !error && (
            ...
            {loading && (
                <div className="flex justify-center items-center py-10">
                    ...
                    <p className="ml-3 text-gray-500 dark:text-gray-400">{t('watchlist_preview_loading')}</p>
                </div>
            )}
            {error && (
            ...
            {!loading && !error && (
                watchlistItems.length > 0 ? (
                                    ...
                                    <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800">
                                        ...
                                        {token && (
                                            <div className="absolute bottom-2 right-2 flex flex-col space-y-1.5 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-200 delay-100 z-20">
                                                ...
                                                <button
                                                    onClick={() => handleAddToWatchlist(item.itemId, item.contentId, item.contentType)}
                                                    ...
                                                    title={t('watchlist_item_add_button_title')}
                                                    aria-label={t('watchlist_item_add_button_aria', { title })}
                                                >
                    ...
                    <div className="text-center p-8 md:p-12 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/30">
                        ...
                        <p className="text-lg text-gray-600 dark:text-gray-400 mb-3">{t('watchlist_preview_empty_title')}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-500 mb-6">
                            {t('watchlist_preview_empty_desc', { username })}
                        </p>
                        <Link to="/" className="bg-yellow-600 hover:bg-yellow-500 text-white font-semibold py-2 px-5 rounded-md transition duration-150 ease-in-out text-sm shadow-sm">
                            {t('watchlist_preview_empty_button')}
                        </Link>
                    </div>
                )
            )}
...
export default WatchlistPreviewPage; 